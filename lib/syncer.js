...
function Syncer(config) {
  this.config = Syncer.sanitizedConfig(config ? config : require(Syncer.ConfigPath))
...
/**
 * Sanitize syncer config object.
 * @param {object} config
 * @return {object}
 */
Syncer.sanitizedConfig = function (config) {
  const clone = _.cloneDeep(config)
  clone.sync.path = clone.sync.path.replace(/^\/+/, '').replace(/\/+$/, '')
  return clone
}
...
Syncer.writeConfig = function (data) {
  return Fs.writeFile(this.ConfigPath, this.makeConfig(data))
}
...
Syncer.prototype.initBsync = function () {
  ...
  this.bsync.watch('**/*').on('change', this.onFileChange.bind(this))
}
...
Syncer.prototype.compileSass = function () {
  ...
  function renderSass(file) {
    return Fs.readFile(file.path, 'utf8')
      .then((data) => sassRender({ data: data || ';', includePaths: [Path.join(process.cwd(), sassConfig.source)] }))
      .catch((err) => { throw err })
  ...
  function writeCssFile(compiled) {
    ...
    return Fs.writeFile(cssPath, compiled.sass.css, 'utf8')
  }
...
Syncer.prototype.existsRemote = function (path) {
  ...
  return this.ftp.list(`/${path}`)
    .then((list) => {
      if (!list.length) {
        throw new Error(`Remote path ${path} doesn't exist!`)
      }
    })
}
...
Syncer.prototype.ensureRemoteDir = function (path) {
  path = this.asRemotePath(path)
  return this.ftp.ensureDir(`/${path}`)
}
 ...
 */
 Syncer.prototype.asLocalPath = function (path) {
   ...
   path = path.replace(new RegExp("^" + this.config.sync.path), ".");
   path = path.replace(/^(\.\/)+/, '')
   return `./${path}`
 }
...
Syncer.prototype.syncDir = function (path) {
  console.log('[SYNC]'.yellow + ' ' + path.white)
