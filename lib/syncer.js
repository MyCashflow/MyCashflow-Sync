...
Syncer.prototype.onFileChange = function (path) {
  ...
  this.workQueue()
    ...
    .catch(() => {
      const err = Error('Upload failed! This usually happens if you have created new deeply nested directories on the local machine, please run sync or watch again.')
      this.exit(err)
    })
}
 ...
 */
 Syncer.prototype.asLocalPath = function (path) {
   path = Path.normalize(path).replace(process.cwd(), '.')
