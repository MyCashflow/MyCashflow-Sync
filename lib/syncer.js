  ...
  this.workQueue()
    .then(() => {
      ...
      if (Match(filename, ['*.{css,html,js}']).length) {
        this.bsync.reload(filename)
      }
    })
    .catch(() => {
      const err = Error('Upload failed! This usually happends if you have created new deeply nested directories on the local machine, please run sync or watch again.')
      this.exit(err)
    })
}
...
Syncer.prototype.isIgnored = function (path) {
  ...
  return Match(path, ignores, { matchBase: true, dot: true }).length
}
