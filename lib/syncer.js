...
Syncer.prototype.initFtp = function () {
  return new Promise((resolve, reject) => {
    ...
    this.ftp.auth(this.config.ftp.user, this.config.ftp.pass, err => {
      if (err) reject(err)
      else resolve()
    })
  })
...
Syncer.prototype.ensureRemoteDir = function (path) {
  ...
  return this.existsRemote(path)
    .catch(() => {
      ...
      return Promise.mapSeries(parts, (_part, index) => {
        const currPath = parts.slice(0, index + 1).join('/')
        return this.createRemoteDir(currPath)
      })
    })
}
