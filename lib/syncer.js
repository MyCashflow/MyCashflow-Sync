...
Syncer.prototype.exit = function (err) {
  if (err) console.log(`${err.name.gray}: ${err.message.red}`)
...
Syncer.prototype.initBsync = function () {
  ...
  this.bsync.init(_.merge({}, BsyncConfig, { proxy: this.config.sync.proxy }))
...
Syncer.prototype.isIgnored = function (path) {
  ...
  return !!Match(path, ignores).length
}
 ...
 */
 Syncer.prototype.asLocalPath = function (path) {
   ...
   if (parts[0] === this.config.sync.remote) {
     parts[0] = '.'
   }
   return parts.join('/')
 }
 ...
 */
 Syncer.prototype.asRemotePath = function (path) {
   const parts = path.split('/')
   if (parts[0] === '.') {
     parts[0] = this.config.sync.remote
   }
   return parts.join('/')
 }
...
Syncer.prototype.download = function (path) {
  console.log('[DOWN]'.gray + ' ' + path.yellow)
...
Syncer.prototype.upload = function (path) {
  console.log('[UPLD]'.gray + ' ' + path.yellow)
...
Syncer.prototype.syncDir = function (path) {
  console.log('[SYNC]'.gray + ' ' + path.yellow)
