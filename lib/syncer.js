...
Syncer.prototype.compileSass = function () {
  ...
  function validateSassConfig() {
    return new Promise((resolve, reject) => {
      if (!sassConfig.source && !sassConfig.dest) {
  ...
  function renderSass(file) {
    return Sass.renderAsync({ file: file.path })
      .catch((err) => { throw err })
      .then((scss) => scss.css.toString())
      .then((css) => { return { file, css } })
  }

  function writeCssFile(compiled) {
    const cssPath = compiled.file.path.replace('.scss', '.css')
      .replace(`${sassConfig.source}`, `${sassConfig.dest}`)
    return Fs.writeFileAsync(cssPath, compiled.css)
  }
...
Syncer.prototype.onFileChange = function (path) {
  path = this.asLocalPath(Path.resolve(path))

  const sassEnabled = !!this.config.sass.source
  const sassChanged = path.indexOf(`./${this.config.sass.source}`) === 0
  if (sassEnabled && sassChanged) {
    this.compileSass().catch((err) => {
      console.log('[SASS]'.white + ' ' + err.formatted.red)
    })
  }
